...
def johnson(n, S):
    ...
    res = bellman_ford(Adj, weight_func, n)
    if res is None:
        return None
    BF_distances, BF_parents = res

    # Dijkstra
    def reweight_func(u, v):
        return W[(u, v)] + BF_distances[u] - BF_distances[v]
    
    for i in range(n):
        dijk_distances, dijk_parents = dijkstra(Adj, reweight_func, i)
        for j in range(n):
            if dijk_distances[j] < INF:
                D[i][j] = dijk_distances[j] - BF_distances[i] + BF_distances[j]
